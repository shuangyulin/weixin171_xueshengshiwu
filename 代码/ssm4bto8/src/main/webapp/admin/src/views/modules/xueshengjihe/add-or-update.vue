<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="集合名称" prop="jihemingcheng">
          <el-input v-model="ruleForm.jihemingcheng" 
              placeholder="集合名称" clearable  :readonly="ro.jihemingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="集合名称" prop="jihemingcheng">
              <el-input v-model="ruleForm.jihemingcheng" 
                placeholder="集合名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="姓名" prop="xingming">
                <el-input
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengjihe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengjiheCrossAddOrUpdateFlag = false;
						 this.parent.search();
