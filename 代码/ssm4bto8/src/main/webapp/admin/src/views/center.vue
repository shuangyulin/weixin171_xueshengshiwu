     ...
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='xuesheng'"  label="学校" prop="xuexiao">
          <el-select v-model="ruleForm.xuexiao" :disabled="true" placeholder="请选择学校">
            <el-option
                v-for="(item,index) in xueshengxuexiaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='xuesheng'"  label="学院" prop="xueyuan">
          <el-select v-model="ruleForm.xueyuan" :disabled="true" placeholder="请选择学院">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='xuesheng'"  label="年级" prop="nianji">
          <el-select v-model="ruleForm.nianji" :disabled="true" placeholder="请选择年级">
            <el-option
      ...
      <el-col :span="24">  
        <el-form-item v-if="flag=='jiaoshi'" label="证件" prop="zhengjian">
          <file-upload
          tip="点击上传证件"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhengjian?ruleForm.zhengjian:''"
          @change="jiaoshizhengjianUploadChange"
          ></file-upload>
        </el-form-item>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  mounted() {
    ...
    this.$http({
      url: `option/xuexiao/xuexiao`,
      method: "get"
    }).then(({ data }) => {
      if (data && data.code === 0) {
        this.xueshengxuexiaoOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    jiaoshizhengjianUploadChange(fileUrls) {
        this.ruleForm.zhengjian = fileUrls;
    },
    onUpdateHandler() {
      ...
      if((!this.ruleForm.mima)&& 'jiaoshi'==this.flag){
        ...
        if(this.ruleForm.zhengjian!=null) {
                this.ruleForm.zhengjian = this.ruleForm.zhengjian.replace(new RegExp(this.$base.url,"g"),"");
        }
